...
import org.modular.playground.catalog.web.graphql.BookUpdateGql;
...
public class BookGraphQLControllerIntegrationTest {
    ...
    private Book createAndTrackBook(BookGql request) {
    ...
    void testGetBookByIdExistingIdShouldReturnOkAndBook() {
        Book book = createAndTrackBook(CatalogTestUtils.createValidBookGql());
        String body = String.format("""
                {
    ...
    void testAdminCanUpdateBookSuccessful() {
        Book book = createAndTrackBook(CatalogTestUtils.createValidBookGql());
        BookUpdateGql updateDTO = new BookUpdateGql();
